...
| CWE-416/415 in a CONTAINER that restores a length/ownership invariant *after* a may-unwind call — `drop_in_place` before `set_len`, `ptr::read` before `mem::forget`, a hole open across `retain`/`dedup`/`drain` | [`panicking_drop`](harness-templates/panicking_drop.rs) | **Miri**, definitive — validated on a paired vuln/patched crate (UB on the vulnerable version, silent on the patch) |
