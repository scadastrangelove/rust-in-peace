...
argument-hint: "<crate-name> [--commit <sha>] [--results <dir>] [--remote <host>] [--engines a,b,c] [--byol codeql]"
allowed-tools:
  - Read
  - Glob
  - Grep
...
**`--byol codeql` (bring-your-own-licence).** CodeQL's CLI is proprietary and is never in the image;
only our MIT queries (`rules/codeql/rust/`) ship. To add CodeQL as a 7th engine, point
`SAST_CODEQL_CLI` at a Rust-capable CodeQL distribution on the box (the dir holding the `codeql`
binary, or the binary itself) — `sast-scan.sh` mounts it read-only at `/opt/codeql`, adds `codeql` to
the engine set, runs `database create --language=rust` + `analyze` over our queries, and drops
`codeql.sarif` into `raw/` where `normalize.py` already ingests it. It stays off unless enabled.

```bash
# CodeQL as the 7th engine (operator supplies the CLI; queries ship with the repo)
ssh <remote-host> 'cd ~/rip-sast && SAST_IMAGE=vuln-pipeline-sast:v1.3 \
  SAST_CODEQL_CLI=~/codeql ./sast-scan.sh h2 https://github.com/hyperium/h2 <commit>'
```

The operator's CLI must be able to resolve the `codeql/rust-all` pack our qlpack depends on (bundled
in the dist, or a prior `codeql pack download`); if those packs live beside the CLI, set
`CODEQL_ADDITIONAL_PACKS` (commonly `/opt/codeql/qlpacks`).
...
**A high cell count is a RANKING problem, not a stop condition** (recall-first, 2026-08-01). The old
rule here was "if cells > 40, stop" — a designed recall leak: it discards enumeration that is already
paid for. Cells are ordered rarest-rule-first, and rarity is the one signal measured to carry
information (E7c: 1.9 hits/crate → 11.1 % discrimination vs 32.5/crate → 0.5 %; and in the h2 pass the
two SMALLEST cells produced both real seeds while the five largest — 527 hits — produced none).

So: spend the agent budget top-down over the ranked list, and **log the unread tail explicitly**
(`cells_unread: N`) so "we ran out of budget" never reads as "the tools found nothing". Never take a
top-N as a filter and never merge cells to fit — the merge is what fuses disjoint trust boundaries
(bringup §5.3) and makes one verdict cover two data paths.
...
- **A verdict is per SITE-GROUP, never per cell.** A cell is a work unit, not a claim. Cells carry
  every site grouped by rule, and a coarsened cell can span several subsystems — h2's `src::panic-surface`
  held 78 sites across three unrelated ones. A single "nothing here" on such a cell silently disposes of
  all 78. So a triage/finder pass must return a disposition **per rule-group** (and name any site it did
  not reach), and a cell counts as read only when every group in it has one. This is what makes coarse
  cells safe; it is not a reporting nicety.
