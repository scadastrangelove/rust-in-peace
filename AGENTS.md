...
- **Interactive skills** (`.Codex/skills/`) — read and write files in the
...
`Codex -p` session; a 429 or 5xx mid-run is retried inside the CLI first,
...
agent then runs `Codex -p` inside its own gVisor container (`--runtime=runsc
...
from `.Codex/settings.json` instead (override in `.Codex/settings.local.json`).
See `docs/pipeline.md#usage-marker`.

**`-e Codex=` and `-e IS_SANDBOX=1` in `agent.py` are load-bearing.**
