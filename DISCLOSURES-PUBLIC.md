...
| Codex (openai/codex) | 2026-08-05 | [#37079](https://github.com/openai/codex/issues/37079) | Low | Open — candidate patch posted 2026-08-07 (same PR-access restriction; fix + regression test posted inline plus a cherry-pickable fork branch); no maintainer response yet | execpolicy `forbidden`/deny rules bypassable by spelling argv[0] as an unregistered path (`/tmp/git` vs `git`) |
| Codex (openai/codex) | 2026-08-05 | [#37080](https://github.com/openai/codex/issues/37080) | Medium | Open — candidate patch posted 2026-08-07 (same PR-access restriction; fix + regression test posted inline plus a cherry-pickable fork branch); no maintainer response yet | MCP client does not cap the HTTP/SSE response body on the default (Legacy) request — malicious MCP server memory-exhaustion DoS |
...
| Chromium / Skia (first-party `rust/exif`) | 2026-08-02 | [issue 541725390](https://issues.chromium.org/issues/541725390) | Low | Resolved — root cause fixed upstream in Skia (2026-08-17, `[rust exif] Avoid repeated ExifIFD traversal`), auto-rolled into Chromium 2026-08-18 | EXIF IFD parser re-parses a repeated sub-IFD pointer with no idempotence guard on one tag arm — quadratic work-amplification DoS |
...
| h2 | 2026-07-26 | [PR #936](https://github.com/hyperium/h2/pull/936) | Medium | Resolved (2026-08-17, merged by `seanmonstar` himself; released in **h2 0.4.16**) | HTTP/2 client accepting server push (the library's own default) could have its connection task panicked by a malicious server via `PUSH_PROMISE` followed by repeated 1xx informational responses on the same reserved stream — double-counted-stream assertion failure |
| h2 | 2026-07-26 | [GHSA-q83h-524g-xf6h](https://github.com/hyperium/hyper/security/advisories/GHSA-q83h-524g-xf6h) | Low | **Resolved — published as a public advisory** (2026-08-17), CWE-400; fixed in **h2 0.4.16** | `h2` accepted and queued empty `DATA` frames without limit — if a stream isn't actively drained, this allows unbounded memory growth or a panic on length overflow |
...
| h2 | 2026-07-26 | [GHSA-8r6j-x8wp-qpm3](https://github.com/hyperium/hyper/security/advisories/GHSA-8r6j-x8wp-qpm3) | Closed by vendor — advisory not published; addressed via public [PR #936](https://github.com/hyperium/h2/pull/936), released in **h2 0.4.16** |
